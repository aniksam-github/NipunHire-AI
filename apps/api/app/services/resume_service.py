...
from app.services.embedding_service import build_profile_text, generate_embedding
...
async def process_and_save_resume(
    ...
    try:
        ...
        # Vector Embedding & FAISS Index Sync
        try:
            profile_text = build_profile_text(profile)
            embedding = generate_embedding(profile_text)
            profile.embedding = embedding
            await vector_search_service.add_profile_to_index(
            ...
            logger.warning("Could not generate or index vector embedding for resume %s: %s", resume.id, vec_exc)
...
async def delete_resume(resume_id: str, candidate_id: str) -> None:
    ...
    if resume.profile_id:
        try:
            await vector_search_service.mark_profile_deleted(
                profile_id=str(resume.profile_id), candidate_id=candidate_id
            )
        except Exception as vec_exc:
            logger.warning("Could not mark profile %s as deleted in vector index: %s", resume.profile_id, vec_exc)
