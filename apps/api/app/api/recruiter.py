...
from app.db.elasticsearch import search_candidates_es
...
# --- Module 0: Elasticsearch Full-Text & Fuzzy Candidate Search ---
@router.get(
    "/search",
    status_code=status.HTTP_200_OK,
    summary="Elasticsearch Full-Text Candidate Search",
    description="Performs ultra-fast BM25 relevance scoring & fuzzy text search across candidate skills, headlines, and resumes."
)
async def search_candidates_full_text(
    q: str,
    limit: int = 10,
    current_user: User = Depends(require_role("recruiter", "admin"))
):
    if not q or not q.strip():
        raise HTTPException(status_code=400, detail="Search query 'q' parameter cannot be empty.")

    results = await search_candidates_es(query_text=q.strip(), limit=limit)
    return {
        "query": q,
        "total_hits": len(results),
        "search_engine": "Elasticsearch 8.x (BM25)",
        "results": results
    }
