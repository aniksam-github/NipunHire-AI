...
def mock_beanie_collection(monkeypatch):
    """Bypass Beanie collection lookup when instantiating Document models in unit tests."""
    monkeypatch.setattr("beanie.Document.get_pymongo_collection", lambda *args, **kwargs: None)
...
async def test_record_audit_event_unit(monkeypatch):
    ...
    dummy_user.id = PydanticObjectId()
